...
export default function HomePage() {
  ...
  const completedTodayQuests = tasks
    .filter((t) => t.completed && t.completedAt && isToday(t.completedAt))
    .slice(0, 5);
  ...
  const weeklyProgressPercent = Math.min(
    100,
    Math.round((todayUnits / dailyTarget) * 100),
  ); // Using today's for now as it's just a visual metric
  ...
  // AuthProvider will handle redirect if no user, but we keep it safe
  if (!user) return null;
      ...
      rightPanel={
        <div className="space-y-6 xl:pr-6">
          ...
          <Card className="flex flex-col items-center justify-center p-6 bg-white border border-gray-100 shadow-sm text-center">
            ...
            <p className="font-extrabold text-3xl text-text-primary mb-1">
              {user?.streakDays || 0}
            </p>
          ...
          {/* Daily Goal Card */}
          <Card
            variant="static-content"
            className="bg-orange-50 border-orange-100 p-6 flex flex-col gap-4"
          >
            ...
            <div className="w-full bg-orange-200/50 rounded-full h-2.5 mb-2">
              <div
                ...
                style={{
                  width: `${Math.min(100, (todayUnits / dailyTarget) * 100)}%`,
                }}
              ></div>
            </div>
      ...
      <div className="space-y-8 pb-10 w-full">
        ...
        <div className={`bg-gradient-to-r ${bannerTheme.bg} rounded-[2rem] p-8 md:p-10 text-white flex justify-between items-center shadow-lg relative overflow-hidden transition-all duration-1000`}>
          ...
          <div className="relative z-10 hidden md:flex items-center justify-center w-32 h-32 bg-white/20 backdrop-blur-sm rounded-full border-4 border-white/30 shadow-xl mr-4 transform hover:scale-105 transition-transform duration-300">
            <Rocket
              size={64}
              className="text-white -rotate-12 translate-x-1 -translate-y-1"
              fill="currentColor"
            />
          </div>
        </div>
                  ...
                  <Link href="/matrix" key={task.id} className="block w-full">
                    <Card className="flex justify-between items-center p-5 border-2 border-gray-100 hover:border-blue-100 hover:shadow-md transition-all group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-text-primary text-base mb-0.5">
                            {task.title}
                          </p>
                  ...
                  <Link href="/matrix" key={task.id} className="block w-full">
                    <Card className="flex justify-between items-center p-5 border-2 border-[#A5ED5B] bg-[#F3FCE8] hover:shadow-md transition-all group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-[#5C940D] text-base mb-0.5">
                            {task.title}
                          </p>
