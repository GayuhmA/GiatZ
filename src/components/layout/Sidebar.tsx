...
const navItems = [
  ...
  { name: 'FOCUS ORBIT', href: '/orbit', iconOutline: SparklesIcon, iconSolid: SparklesIconSolid },
  { name: 'TASK MATRIX', href: '/matrix', iconOutline: QueueListIcon, iconSolid: QueueListIconSolid },
  { name: 'EXPLORER', href: '/explorer', iconOutline: GlobeAltIcon, iconSolid: GlobeAltIconSolid },
  { name: 'TRAIN CAMP', href: '/camp', iconOutline: AcademicCapIcon, iconSolid: AcademicCapIconSolid },
];
  ...
  return (
    <aside className="fixed bottom-0 left-0 w-full h-[72px] md:h-screen md:w-[240px] bg-bg-card flex flex-row md:flex-col border-t md:border-t-0 md:border-r border-border z-50">
      ...
      <div className="hidden md:flex pt-10 pb-8 px-8 flex-row items-center gap-3">
        ...
        <h1 className="text-primary font-extrabold text-2xl tracking-wide">GIATZ</h1>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`flex flex-col md:flex-row items-center justify-center md:justify-start gap-1 md:gap-4 px-2 md:px-5 py-2 md:py-3.5 rounded-xl md:rounded-full text-[10px] md:text-[15px] font-bold uppercase tracking-wide transition-all ${
                isActive
                  ? 'bg-primary-light border-2 border-orange-100 text-primary'
                  : 'border-2 border-transparent text-slate-500 hover:bg-gray-50'
              }`}
            >
              <Icon className="w-6 h-6 md:w-[22px] md:h-[22px]" />
