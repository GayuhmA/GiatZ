      ...
      try {
        ...
        if (!statsDoc.exists()) {
          ...
          await setDoc(statsRef, {
            ...
            focusSeconds:
              type === "focus" ? additionalData.focusSeconds || 0 : 0,
          });
          ...
          if (user.lastActiveAt) {
            const lastActiveDate = user.lastActiveAt.toDate
              ? user.lastActiveAt.toDate()
              : new Date(user.lastActiveAt as any);
          ...
          // Update local state
          setUser(
            { ...user, streakDays: newStreak, lastActiveAt: Timestamp.now() },
            useAuthStore.getState().firebaseUser,
          );
        } else {
